...
function et_eC(param)
{
	...
	if(et_first && !(false || et_optInActive) && !document.getElementById('et_image'))
	{
...
function et_eC_Wrapper( param, et_pn, et_ar, et_il, et_ur, et_ta, et_tv, et_to, et_ts, et_cu, et_ba, et_lp, et_tr, et_tg, et_sb, et_ref )
{
    et_up = '';
    var secureKey;
    ...
    if(!param.length)
	{
		et_pagename     = param.et_pagename     ? et_escape(param.et_pagename)     : '';
		et_areas        = param.et_areas        ? et_escape(param.et_areas)        : '';
		et_ilevel       = param.et_ilevel       ? et_escape(param.et_ilevel)       : 0; 
		et_url          = param.et_url          ? et_escape(param.et_url)          : '';
		et_target       = param.et_target       ? et_escape(param.et_target)       : '';
		et_tval         = param.et_tval         ? et_escape(param.et_tval)         : '';
		et_tonr         = param.et_tonr         ? et_escape(param.et_tonr)         : ''; 
		et_tsale        = param.et_tsale        ? et_escape(param.et_tsale)        : 0;
		et_cust         = (param.et_cust && typeof(param.et_cust) == 'number') ? param.et_cust : 0;
		et_basket       = param.et_basket       ? et_escape(param.et_basket)       : ''; 
		et_lpage        = param.et_lpage        ? et_escape(param.et_lpage)        : '';
		et_trig         = param.et_trigger      ? et_escape(param.et_trigger)      : '';
		et_tag          = param.et_tag          ? et_escape(param.et_tag)          : '';
		et_organisation = param.et_organisation ? et_escape(param.et_organisation) : '';
		et_demographic  = param.et_demographic  ? et_escape(param.et_demographic)  : '';
        et_sub          = param.et_sub          ? et_escape(param.et_sub)          : '';
		et_referrer 	= param.et_ref			? et_escape(param.et_ref)			 : et_referrer;
	...
	else
	{
		if(et_pn == 'null') et_pn = '';
 		...
 		if(et_tg == 'null') et_tg = '';
 		if(et_sb == 'null') et_sb = '';
		if(et_ref == 'null') et_ref = '';
 	
		et_pagename     = et_pn ? et_escape(et_pn): '';
		et_areas        = et_ar ? et_escape(et_ar) : '';
		et_ilevel       = et_il ? et_escape(et_il) : 0; 
		et_url          = et_ur ? et_escape(et_ur) : '';
		et_target       = et_ta ? et_escape(et_ta) : '';
		et_tval         = et_tv ? et_escape(et_tv) : '';
		et_tonr         = et_to ? et_escape(et_to) : ''; 
		et_tsale        = et_ts ? et_escape(et_ts) : 0;
		et_cust         = et_cu ? et_cu : 0;
		et_basket       = et_ba ? et_escape(et_ba) : ''; 
		et_lpage        = et_lp ? et_escape(et_lp) : '';
		et_trig         = et_tr ? et_escape(et_tr) : '';
		et_tag          = et_tg ? et_escape(et_tg) : '';
        et_sub          = et_sb ? et_escape(et_sb) : '';
		et_referrer		= et_ref ? et_escape(et_ref) : et_referrer;	
...
{
	for (var i=0; i < et_d_anchors.length; i++)
	{
		et_d_anchors[i].position = i;
		et_addEvent(et_d_anchors[i], 'mousedown', et_getLink);
	}
...
// Content vom Element
// Position im DOM
// Absolute Klickposition der Maus Links
// Absolute Klickposition der Maus Oben
// 
...
function et_sendData( a, n, c, p, x, y, s, t )
{
	...
	var oh = et_escape(domain + et_spPage(window.location.pathname) + et_spPage(s));
	...
	var m = et_d_anchors.length + ( et_links ? et_d.getElementsByTagName('input').length + et_d.getElementsByTagName('select').length : 0 );
	
	par = 'et='+et_et+'&n='+oh+'&i='+et_escape(pagename)+'&easy='+easy+'&p='+p+'&m='+m+'&h='+et_divHash(n)+'&c='+et_divHash(c)+'&x='+x+'&y='+y+'&t='+t;
...
function etEvent(secure,host) 
{
	...
	var sendEvent = function(object)
	{
		...
		if(object.category)
			et_parameters += '&et_cat=' + et_escape(object.category); 

		if(object.item)
			et_parameters += '&et_item=' + et_escape(object.item); 

		if(object.action)
			et_parameters += '&et_action=' + et_escape(object.action); 

		if(object.tags)
			et_parameters += '&et_tags=' + et_escape(object.tags); 

		if(object.value)
			et_parameters += '&et_value=' + et_escape(object.value); 

		if(typeof(et_pagename) != 'undefined' && typeof(et_pagename) != 'unknown')
		{
			et_parameters += '&et_pagename=' + et_escape(et_pagename);
		...
		else
		{
			...
			et_parameters += '&et_url='+et_escape(et[0]);
		}
		...
		et_parameters += '&scolor=' + et_escape(et_sc);
		et_parameters += '&swidth=' + et_escape(et_sw);
...
var et_coid = '1f41767cde8b6b33efc7c9073d205765';

function et_getDeliveryHash()
{
	return 'TO9tiqEydT+hdL91R+plCQ==';
}
...
function et_createScriptTag(url)
{
...
function et_getCookieValue(cName)
{
    return document.cookie.replace(new RegExp('(?:(?:^|.*;)\\s*' + cName.replace(/[\-\.\+\*]/g, '\\$&') + '\\s*\\=\\s*([^;]*).*$)|^.*$'), '$1') || ''; 
}

function et_getFpcContent()
{
    var cookies = ['_et_coid'];
    var result = [];
    
    for(var i = 0; i < cookies.length; i++)
    {
        var cookieValue = et_getCookieValue(cookies[i]); 
        if(cookieValue)
    	{
    		result.push(cookies[i] + '=' + cookieValue);
    	}
    }
...
function et_appendCntImage(cntImage)
{
    	var append_to = document.getElementById('et_image');
	if(append_to)
	{
		append_to.parentNode.insertBefore(cntImage, append_to.nextSibling);
	}
	else
	{
		var body = document.getElementsByTagName('body')[0]; 
		body.insertBefore(cntImage, body.lastChild);
	}
}

function et_cookiesEnabled()
{
    document.cookie = 'ist=an;path=/';
    var an = document.cookie.indexOf('ist=an') > -1;
...
{
    return '&et_fpc=' + et_escape(et_getFpcContent()) + '&et_coid='+et_coid+'&et_ca='+(et_ca?1:0)+'&et_cd='+window.location.hostname;
}var et_ca = et_cookiesEnabled();
// plugin detection;
var et_pd_maxjs	= 19;
var et_pd_maxfl	= 15;
var et_pd_maxsh	= 15;
...
var et_d_anchors	= et_d.getElementsByTagName('a');
...
function et_iO()
{
	...
	else if((et_location.match(/.et_overlay=1/gi))||(et_d.cookie.match(/et_overlay=1/))||(et_d.cookie.match(/et_overlay=2/)))
	{
		...
		var url = et_host+'et_overlay_show.php'+'?et='+et_et+'&n='+et_escape(et_thisHref)+'&i='+et_escape(pagename)+'&easy='+easy+'&o=' + et_overlay + et_liveSwitch +'&t='+new Date().getTime(); 
		if((url.length + window.location.toString().length) < et_maxUrlLength)
			url = url+'&r='+et_escape(window.location);
...
ET_Event = new etEvent('HD3WaK', et_server);
function _etc()
{
	var c = "";
